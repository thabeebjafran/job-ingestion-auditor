    ...
    for i, item in enumerate(items[:15]):
        ...
        try:
            ...
            if is_scam:
                ...
                apply_row = [company_res, role_res, career_url_res or 'N/A', 'Applied', visa_status, date_added, follow_up_date, tech_stack, notes]
    ...
    if (raw_text or image_bytes) and space_name:
        try:
            ...
            if is_scam:
                ...
                visa_status = 'Requires Sponsorship' if ('sponsor' in visa.lower() or 'yes' in visa.lower()) else 'Not Required'
                apply_row = [company, role, career_url or 'N/A', 'Applied', visa_status, date_added, follow_up_date, tech_stack, notes]
                sheet_ok = append_to_sheet('Sheet1', apply_row)
                sheet_info = 'Logged to Sheet1 (Database)' if sheet_ok else 'Saved'
